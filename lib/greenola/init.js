...
export function initGreenolaApp() {
      ...
      function restoreState() {
        try {
          ...
          if (saved.companyCode) state.companyCode = saved.companyCode;
          if (saved.startDate) state.startDate = saved.startDate;
          if (saved.deliveryAddress) state.deliveryAddress = saved.deliveryAddress;
      ...
      function validateForm() {
        ...
        submitBtn.disabled = !(nameValid && phoneValid && companyValid && emailValid);
      }
      [nameInput, phoneInput, companyInput, emailInput].forEach(el => el.addEventListener('input', validateForm));
      ...
      // ═══ MENU SCREEN LOGIC ═══════════════════════════════════════════
      // MEALS must be initialized before any restore path calls renderMenu()
      ...
      // Restore journey if user refreshed mid-flow (after MEALS/renderMenu exist)
      if (state._restoredScreen && state._restoredScreen > 1 && state.lead) {
        ...
        if (state._restoredScreen >= 3 && !window.__menuRendered) {
          renderMenu();
          window.__menuRendered = true;
        }
      } else {
      ...
      // Screen 5: Pay button — create checkout session via payment gateway
      const payBtn = document.getElementById('payBtn');
      if (payBtn) payBtn.addEventListener('click', async () => {
        ...
        if (state.lead?.id) {
          logActivity(state.lead.id, 'pay_clicked', { method: state.selectedPaymentMethod });
        }

        try {
          if (!isValidEmail(state.lead?.email)) {
            toast(t('login_email_invalid'));
            payBtn.disabled = false;
            if (lbl) lbl.textContent = original;
            return;
          }

          const payload = buildCheckoutPayload(state, pricing);
          const checkoutUrl = cfg.checkoutApiUrl || '/api/payment/checkout';
          ...
          if (!response.ok) {
            console.error('[Greenola B2B] Checkout failed:', result);
            throw new Error(result.error || result.message || t('pay_error'));
          }

          if (result.hyperpay) {
            console.log('[Greenola B2B] HyperPay checkout payload sent:', result.hyperpay);
          }

          const paymentUrl = result.paymentUrl || result.data?.paymentUrl || null;
          const checkoutId = result.checkoutId || result.data?.checkoutId || null;
          const orderId = result.orderId || result.data?.orderId || null;
          const supportedBrands =
            result.supportedBrands || result.data?.supportedBrands || null;
          const userPays = pricing.userPays ?? pricing.afterDiscount ?? pricing.baseTotal;

          if (state.lead?.id) {
            logActivity(state.lead.id, 'checkout_created', {
          ...
          if (paymentUrl || checkoutId) {
            if (lbl) lbl.textContent = t('pay_redirecting');
            try {
              sessionStorage.setItem('greenola_payment_execution', JSON.stringify({
                checkoutId,
                orderId: orderId ? String(orderId) : undefined,
                billing: payload.billing_info,
                customer: {
                  givenName: payload.customer_info?.given_name,
                  surname: payload.customer_info?.surname,
                  email: payload.customer_info?.email,
                  mobile: payload.customer_info?.mobile_number,
                },
              }));
            } catch (storageErr) {
              console.warn('[Greenola B2B] Could not persist payment execution context:', storageErr);
            }
            const params = new URLSearchParams();
            if (paymentUrl) params.set('paymentUrl', paymentUrl);
            if (checkoutId) params.set('checkoutId', checkoutId);
            if (orderId) params.set('orderId', String(orderId));
            params.set('amount', String(userPays));
            params.set('method', state.selectedPaymentMethod);
            if (supportedBrands) params.set('supportedBrands', String(supportedBrands));
          ...
          successParams.set('amount', String(userPays));
          window.location.href = `/payment/success?${successParams.toString()}`;
        } catch (err) {
          console.error('[Greenola B2B] Checkout failed:', err);
          const reason = encodeURIComponent(err?.message || t('pay_error'));
          window.location.href = `/payment/failed?reason=${reason}`;
        } finally {
          if (lbl) lbl.textContent = original;
          payBtn.disabled = false;
        }
